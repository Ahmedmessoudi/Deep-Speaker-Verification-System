...
if page == "Compare & Verify":
    ...
    if uploaded_1 and uploaded_2 and st.button("Compare ECAPA vs XVector"):
        try:
            files = {
                'file1': (uploaded_1.name, uploaded_1.getvalue()),
                'file2': (uploaded_2.name, uploaded_2.getvalue())
            ...
            if r.status_code != 200:
                ...
                xvector = models["xvector"]

                if output_choice == "Plots":
                    # Plot original similarity bar
                    bar_fig = go.Figure()
                    bar_fig.add_trace(go.Bar(name="ECAPA-TDNN", x=["Original pair"], y=[ecapa["original_similarity"]]))
                    bar_fig.add_trace(go.Bar(name="X-Vector", x=["Original pair"], y=[xvector["original_similarity"]]))
                    bar_fig.update_layout(
                        barmode="group",
                        title="Original Pair Similarity: ECAPA vs X-Vector",
                    ...
                    snr_fig.add_trace(go.Scatter(
                        x=[item["snr_db"] for item in xvector["snr_curve"]],
                        y=[item["similarity"] for item in xvector["snr_curve"]],
                        mode="lines+markers",
                        name="X-Vector",
                    ))
                    snr_fig.update_layout(
                        title="Similarity vs SNR",
                        xaxis_title="SNR (dB)",
                        yaxis_title="Cosine Similarity",
                        template="plotly_white",
                        height=450,
                    ...
                    # Tables view (default)
                    st.subheader("Original Similarities")
                    st.table([
                        {
                            "Model": "ECAPA-TDNN", 
                        ...
                        {
                            "Model": "X-Vector", 
                            "Original Similarity": f"{xvector['original_similarity']:.4f}",
                            "Same Speaker (Match)": xvector['original_similarity'] >= threshold
                        },
                    ])

                    # Build combined SNR table
                    snr_rows = []
                    # assume both curves have same snr_db ordering
                    for i, item in enumerate(ecapa["snr_curve"]):
                        snr_db = item["snr_db"]
                        ecapa_sim = item["similarity"]
                        xv_sim = xvector["snr_curve"][i]["similarity"] if i < len(xvector["snr_curve"]) else None
                        snr_rows.append({
                            "SNR (dB)": snr_db,
                            "ECAPA-TDNN Similarity": f"{ecapa_sim:.4f}",
                            "ECAPA Match": ecapa_sim >= threshold,
                            "X-Vector Similarity": f"{xv_sim:.4f}" if xv_sim is not None else "N/A",
                            "X-Vector Match": (xv_sim >= threshold) if xv_sim is not None else False
                        })

                    st.subheader("Similarity vs SNR (table)")
                    st.table(snr_rows)
        except Exception as e:
                        ...
                        for idx, (nf, audio_bytes) in enumerate(st.session_state["aug_results"]):
                            ...
                            if st.button(f"Test against original #{idx+1}", key=button_key):
                                try:
                                    ...
                                    with st.spinner("Calling verify API..."):
                                        r_ecapa = requests.post(f"{api_url}/verify", files=files, params={"threshold": threshold, "model_type": "ecapa_tdnn"})
                                        r_xvec = requests.post(f"{api_url}/verify", files=files, params={"threshold": threshold, "model_type": "xvector"})
                                    ...
                                    # Display X-Vector
                                    if r_xvec.status_code == 200:
                                        j_xvec = r_xvec.json()
                                        msg = f"**X-Vector**: Similarity: {j_xvec['similarity_score']:.4f} — Same speaker: {j_xvec['is_same_speaker']} (Threshold: {threshold})"
                                        if j_xvec['is_same_speaker']:
                                            st.success(msg)
                                        else:
                                            st.error(msg)
                                    else:
                                        st.error(f"X-Vector verify API error: {r_xvec.status_code}")
